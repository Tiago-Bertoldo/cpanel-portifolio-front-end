...
export default function Finish({ titleEdit, valueButton , singEdit}) {
  const { useStore } = useContext(AuthLogin);
  const navigate = useNavigate();
  const ARRAY_VALUE_MAX_TAGS = 6;
  const [formValues, setFormValues] = useState([]);
  console.log(singEdit)
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const handleValidateDate = (e) => {
    ...
    if (tagsCreated.length !== 0) {
      ...
      console.log('ESSE VALOR DEVE SER ENVIADO PARA API PARA DEPOIS CHAMAR O RESTO', formValues)
      setTagsCreated([])
      setApiDate();
      return;
        ...
        <div className="form-container-projet">
          ...
          <div className="form-container-projet-form">
            <div className="form-container-projet-form__left">
              <ul>
                <li>
                  <label htmlFor="projetname">ESSE Nom du projet : </label>
                  <input
                    type="text"
                    name="nom"
                    id="nom"
